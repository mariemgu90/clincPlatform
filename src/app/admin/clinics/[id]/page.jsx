...
export default function ClinicDetailPage() {
      ...
      await Promise.all([
        ...toUnlink.map(serviceId =>
          fetch(`/api/services/${serviceId}`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ clinic: null }),
          })
        ),
        ...toLink.map(serviceId =>
          fetch(`/api/services/${serviceId}`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ clinic: clinicId }),
  ...
  if (session?.user?.role !== 'ADMIN') {
    return <div className="min-h-screen flex items-center justify-center"><p className="text-xl">Access Denied</p></div>;
  }
