...
// import { usePatients, useCreatePatient, useUpdatePatient } from '@/lib/api';
...
export default function PatientsPage() {
  ...
  const [patients, setPatients] = useState(patientsQuery.data || []);
  ...
  useEffect(() => {
    if (patientsQuery.data) {
      let filteredData = patientsQuery.data;

      // If user is not ADMIN, filter by clinic ID
      if (currentUser?.role !== 'ADMIN' && currentUser?.clinicId) {
        filteredData = filteredData.filter(patient => patient.clinicId === currentUser.clinicId);
      }

      setPatients(filteredData);
    }
  }, [patientsQuery.data, currentUser]);

  const createPatient = async (data) => {
    return await createPatientMutation.mutateAsync(data);
  };

  const updatePatient = async (id, data) => {
    return await updatePatientMutation.mutateAsync({ id, data });
  };
            ...
            {selectedPatient && !isEditing ? (
              ...
              <div className="p-6">
                <PatientForm
                  ...
                  onCreatePatient={(data) => createPatient(data)}
                  onUpdatePatient={(id, data) => updatePatient(id, data)}
                  onCancel={() => {
                    setShowModal(false);
                    setSelectedPatient(null);
                    setIsEditing(false);
